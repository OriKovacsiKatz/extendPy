...
# Usage: runtime binding of python packages
#
# initially intended for Python q kdb+ (KX).
# initially intended for Utility module for py.q embedPy framework.
...
##########################################################
# Internal Functions
# if library used with kdb+ not to be used in kdb
##########################################################
...
class MemberGroup(enum.IntEnum):
    MODULE = 0,
    CLASS = 1,
    DATA = 2,
    PROPERTY = 3,
...
class Member:
    ...
    def get_members(self, predicate=None):
        return [Member(m[1], m[0], self.grp) for m in inspect.getmembers(self.obj, predicate)]

    def get_modules(self):
        return [m for m in self.get_members(inspect.ismodule) if _is_pub(c.name)]

    def get_classes(self):
        return [c for c in self.get_members(inspect.isclass) if _is_pub(c.name)]

    def get_data(self):
        if self.grp == MemberGroup.CLASS:
            data_info = [DataInfo(a) for a in inspect.classify_class_attrs(self.obj)
                         if _is_pub(a.name) and a.kind == 'data']
            return data_info
        if self.grp == MemberGroup.MODULE:
            ...
            for m in members:
                ...
                if m.grp == MemberGroup.DATA:
                    kind = 'data'
                    base = _obj_cls(m.obj)
                    info = DataInfo(inspect.Attribute(m.name, kind, base, m.obj))
    ...
    def get_methods(self):
        ...
        if self.grp == MemberGroup.MODULE:
            method_info = []
            members = self.get_members(inspect.isroutine)
            for m in members:
                if not _is_pub(m.name):
                    continue
                if m.grp == MemberGroup.FUNCTION:
                    kind = 'builtin method' if isbuiltin(m.obj) else 'base method'
                    info = FuncInfo(inspect.Attribute(m.name, kind, self.obj, m.obj))
    ...
    def get_properties(self):
        if self.grp == MemberGroup.CLASS:
            property_info = [PropertyInfo(a) for a in inspect.classify_class_attrs(self.obj)
                             if _is_pub(a.name) and a.kind == 'property']
...
class ParamInfo(Member):
    def __init__(self, param: inspect.Parameter):
        ...
        self.has_default = param.default is not inspect.Parameter.empty
...
def _module_info(module):
    ...
    mem = Member(module)
    classes = {}
    for cls in mem.get_members(inspect.isclass):
...
def _class_info(obj):
    if inspect.isclass(obj):
        obj = Member(obj)
    if not inspect.isclass(obj.typ):
        return
    attributes = OrderedDict()
...
def _obj_group(obj):
    ...
    elif inspect.isclass(obj) or hasattr(obj, '__slots__'):
